-- ============================================================
-- EdgeKeeper — Migration 022
-- Free tier lifetime cap: 7 -> 15 (a fuller "taste of Marcus" for conversion).
-- Also fixes a pre-existing bug: 'professional' (Private Office) fell through to the
-- 500 cap instead of effectively unlimited.
-- ============================================================
...
BEGIN
  ...
  -- Free plan: 15 total lifetime messages, no grace buffer
  IF v_plan = 'free' OR v_plan IS NULL THEN
    ...
    RETURN QUERY SELECT v_lifetime, (v_lifetime > 15), FALSE;
    RETURN;
  END IF;
